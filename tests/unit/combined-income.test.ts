...
describe('combined-income — מס הכנסה: ההכנסה העצמאית יושבת על השכר', () => {
  ...
  it('ההכנסה הצדדית ממוסה במדרגה השולית שמעל השכר (לא מתחילת הסולם)', () => {
    ...
    // סה"כ 20,000 × 18% = 3,600 ; רכיב הב"ל בלבד: 20,000 × 12.83% = 2,566.
    // ניכוי 52% (על רכיב הב"ל בלבד, סעיף 47א) = 1,334.32.
    // ההכנסה הצדדית החייבת (אחרי ניכוי) = 20,000 − 1,334.32 = 18,665.68,
    // יושבת מ-300,000 עד 318,665.68:
    // 1,200 ₪ ראשונים ב-31% (עד 301,200), היתרה (17,465.68) ב-35%.
    const expected = 1_200 * 0.31 + 17_465.68 * 0.35; // 6,484.99
    expect(r.bituachLeumiDeduction).toBeCloseTo(1_334.32, 2);
  ...
  it('צד גבוה שחוצה מדרגה — ממוסה בשתי מדרגות שונות', () => {
    ...
    // ב"ל: השכר מעל 7,703 → סה"כ 100,000 × 18% = 18,000;
    // רכיב הב"ל בלבד: 100,000 × 12.83% = 12,830 ; ניכוי 52% = 6,671.60.
    // ההכנסה הצדדית החייבת (אחרי ניכוי) = 93,328.40, יושבת מ-228,000:
    // 228,000 → 301,200 ב-31% (73,200), היתרה (20,128.40) ב-35%.
    const expected = 73_200 * 0.31 + 20_128.4 * 0.35; // 29,736.94
    expect(r.bituachLeumiDeduction).toBeCloseTo(6_671.6, 2);
    expect(r.taxOnSideIncome).toBeCloseTo(expected, 0);
  });
...
describe('combined-income — אפס שכיר = עצמאי טהור (הצלבה למחשבון net)', () => {
  it('מס הכנסה תואם את calculateBracketTax על ההכנסה העצמאית פחות ניכוי ה-52%', () => {
    ...
    //   סה"כ: 7,703 × 7.7% + (12,500−7,703) × 18% = 1,354.882/חודש × 12
    //   רכיב הב"ל בלבד: 7,703 × 4.47% + (12,500−7,703) × 12.83%
    //   הניכוי (סעיף 47א) = 52% מרכיב הב"ל בלבד.
    const monthlyBL = 7_703 * 0.077 + (150_000 / 12 - 7_703) * 0.18;
    const expectedBL = monthlyBL * 12;
    const monthlyNIOnly = 7_703 * 0.0447 + (150_000 / 12 - 7_703) * 0.1283;
    const expectedDeduction = monthlyNIOnly * 12 * 0.52;
...
describe('combined-income — ניכוי 52% מדמי הב"ל (סעיף 47א)', () => {
  it('הניכוי מקטין את מס ההכנסה על ההכנסה הצדדית מול תרחיש זהה ללא ניכוי', () => {
    ...
    // ב"ל על הצד: השכר מעל 7,703 → סה"כ 80,000 × 18% = 14,400;
    // רכיב הב"ל בלבד: 80,000 × 12.83% = 10,264 ; ניכוי 52% = 5,337.28.
    const r = calculateCombinedIncome({
      monthlyGrossSalary: 12_000,
      annualSelfEmployedIncome: 80_000,
      creditPoints: 2.25,
    ...
    expect(r.bituachLeumiDeduction).toBeCloseTo(5_337.28, 2);
    ...
    // מס צד עם הניכוי = מס(144,000 + (80,000−5,337.28)) − מס(144,000):
    // 74,662.72 במדרגת 20% = 14,932.54.
    const sideTaxWithDeduction = (80_000 - 5_337.28) * 0.2; // 14,932.54
