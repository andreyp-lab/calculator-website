...
function CurrencyTooltip({ active, payload, label }: {
  ...
  return (
    <div className="bg-paper border border-ink/15 rounded-none p-3 shadow-lg text-right text-sm" dir="rtl">
...
function Alerts({ items, variant = 'amber' }: { items: string[]; variant?: 'amber' | 'blue' | 'red' }) {
  ...
  return (
    <div className={`border rounded-none p-4 space-y-2 ${styles[variant]}`}>
...
function TenureTableWidget({ sector, partTime, currentYears }: {
  ...
  return (
    <div className="bg-cream-2 border border-ink/15 rounded-none overflow-hidden">
      <button
        type="button"
        onClick={() => setOpen(!open)}
        className="w-full flex items-center justify-between px-4 py-3 hover:bg-paper-hover transition"
      >
        <span className="text-sm font-medium text-ink">
          טבלת דמי הבראה לפי וותק — {sector === 'public' ? 'מגזר ציבורי' : 'מגזר פרטי'}
        </span>
        {open ? (
          <ChevronUp className="w-4 h-4 text-ink/70" />
        ) : (
          <ChevronDown className="w-4 h-4 text-ink/70" />
        )}
      </button>
      {open && (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-cream">
                <th className="px-3 py-2 text-right font-semibold text-ink">וותק</th>
                <th className="px-3 py-2 text-center font-semibold text-ink">ימים</th>
                <th className="px-3 py-2 text-center font-semibold text-ink">תעריף/יום</th>
                <th className="px-3 py-2 text-center font-semibold text-ink">סכום שנתי</th>
              </tr>
            </thead>
            <tbody>
              {rows.map((row, i) => {
  ...
  return (
    <div className="bg-paper border border-ink/15 rounded-none p-6 space-y-5">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          <label className="block text-sm font-medium text-ink/70 mb-1">חודשים נוספים</label>
          <select
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium text-ink/70 mb-1">מגזר</label>
          <select
            value={input.sector}
            onChange={(e) => {
        ...
        <div>
          ...
          <div className="flex items-center gap-2 mt-2">
            ...
            />
            <span className="text-sm font-semibold text-ink w-10 text-center">
      ...
      {showIndustry && (
        <div>
          <label className="block text-sm font-medium text-ink/70 mb-1">ענף / הסכם קיבוצי</label>
          <select
            ...
            <div dir="ltr">
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={chartData}>
                  ...
                  <Bar dataKey="amount" name="דמי הבראה שנתיים" fill="#102219" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
    ...
    <div className="space-y-4">
      ...
      {result.isEligible && (
        <>
          ...
          <div className="bg-paper border border-ink/15 rounded-none p-5">
            <h3 className="font-bold text-ink mb-4">הרכב הסכום</h3>
            <div dir="ltr">
...
function RetroactiveTab({ input, onChange }: {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-paper border border-ink/15 rounded-none p-6 space-y-5">
        <h2 className="text-lg font-bold text-ink">נתוני תביעה רטרואקטיבית</h2>
      ...
      {retroResult.isEligible ? (
        <>
          ...
          <div className="bg-paper border border-ink/15 rounded-none overflow-hidden">
            <h3 className="font-bold text-ink px-5 py-3 border-b border-ink/15">
              פירוט לפי שנה
            </h3>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
          ...
          {chartData.length > 1 && (
            <div className="bg-paper border border-ink/15 rounded-none p-5">
              <h3 className="font-bold text-ink mb-4">ברוטו לעומת עם ריבית הלנה</h3>
              <div dir="ltr">
              <ResponsiveContainer width="100%" height={200}>
                <LineChart data={chartData}>
                  ...
                  <Tooltip content={<CurrencyTooltip />} />
                  <Legend />
                  <Line type="monotone" dataKey="ברוטו" stroke="#102219" strokeWidth={2} dot />
          ...
          <div className="bg-cream-2 border border-ink/15 rounded-none p-4">
            <p className="text-sm text-ink font-medium mb-1">הערת התיישנות</p>
            <p className="text-sm text-ink/80">{retroResult.statuteNote}</p>
          </div>

          <div className="bg-cream-2 border border-ink/15 rounded-none p-4 space-y-2">
            <h4 className="text-sm font-bold text-ink">כיצד לתבוע?</h4>
...
function TerminationTab({ input, onChange }: {
  ...
  return (
    <div className="space-y-4">
      <div className="bg-cream-2 border border-ink/15 rounded-none p-4">
        <h3 className="font-bold text-ink mb-1">דמי הבראה בעת סיום עבודה</h3>
      ...
      {result.isEligible ? (
        <>
          ...
          <div className="bg-cream-2 border border-ink/15 rounded-none p-4 space-y-2">
            <h4 className="text-sm font-bold text-ink">זכויות נוספות בגמר חשבון</h4>
            <ul className="text-xs text-ink/70 space-y-1.5">
...
function CompareTab({ input, onChange }: {
  ...
  return (
    <div className="space-y-4">
        ...
        <div dir="ltr">
          <ResponsiveContainer width="100%" height={260}>
            <BarChart data={comparisonData} layout="vertical" margin={{ right: 30 }}>
              ...
              <Tooltip content={<CurrencyTooltip />} />
              <Bar dataKey="annualAmount" name="דמי הבראה שנתיים" fill="#102219" radius={[0, 4, 4, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
        ...
        <div dir="ltr">
          <ResponsiveContainer width="100%" height={220}>
            <LineChart data={accumulationData}>
              ...
              <Tooltip content={<CurrencyTooltip />} />
              <Legend />
              <Line type="monotone" dataKey="מגזר פרטי" stroke="#102219" strokeWidth={2} dot={false} />
      ...
      <div className="bg-cream-2 border border-ink/15 rounded-none p-4 space-y-2">
        <h4 className="text-sm font-bold text-ink">פרטי התעריפים 2026</h4>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
