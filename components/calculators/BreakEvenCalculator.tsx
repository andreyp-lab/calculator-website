    ...
    <div className="grid lg:grid-cols-5 gap-6">
      <div className="lg:col-span-3 bg-paper border border-ink/15 rounded-none p-6">
        ...
        <div className="space-y-5">
          <div>
            <label htmlFor="breakevencalculator-4d9e9d" className="block text-sm font-medium text-ink/70 mb-2">
              הוצאות קבועות חודשיות (₪)
            </label>
            <input id="breakevencalculator-4d9e9d"
              type="number"
              min={0}
              step={500}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label htmlFor="breakevencalculator-1eaaf4" className="block text-sm font-medium text-ink/70 mb-2">
                מחיר ליחידה (₪)
              </label>
              <input id="breakevencalculator-1eaaf4"
                type="number"
                min={0}
            ...
            <div>
              <label htmlFor="breakevencalculator-85c9ae" className="block text-sm font-medium text-ink/70 mb-2">
                עלות משתנה ליחידה (₪)
              </label>
              <input id="breakevencalculator-85c9ae"
                type="number"
                min={0}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label htmlFor="breakevencalculator-bad937" className="block text-sm font-medium text-ink/70 mb-2">
                יחידות צפויות (חודשי)
              </label>
              <input id="breakevencalculator-bad937"
                type="number"
                min={0}
            ...
            <div>
              <label htmlFor="breakevencalculator-f6f8c4" className="block text-sm font-medium text-ink/70 mb-2">
                רווח מטרה חודשי (₪)
              </label>
              <input id="breakevencalculator-f6f8c4"
                type="number"
                min={0}
      ...
      <div className="lg:col-span-2 space-y-4">
        ...
        <div className="bg-cream-2 border border-ink/15 rounded-none p-4 space-y-2 text-sm">
          <div className="flex justify-between">
            <span className="text-ink/70">תרומה ליחידה:</span>
            <span className="font-medium">{formatCurrency(result.contributionPerUnit)}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-ink/70">שיעור תרומה:</span>
          ...
          {input.expectedUnits ? (
            <>
              <div className="flex justify-between pt-2 border-t border-ink/15">
                <span className="text-ink/70">מרווח ביטחון:</span>
                <span
                  className={`font-medium ${result.marginOfSafetyPct >= 0 ? 'text-emerald-800' : 'text-red-700'}`}
                >
                  {result.marginOfSafetyPct.toFixed(1)}%
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-ink/70">רווח חודשי צפוי:</span>
                <span
                  className={`font-bold ${result.expectedProfit >= 0 ? 'text-emerald-800' : 'text-red-700'}`}
                >
          ...
          {input.targetProfit ? (
            <div className="flex justify-between pt-2 border-t border-ink/15">
              <span className="text-ink/70">יחידות לרווח מטרה:</span>
