...
const PIE_COLORS = ['#102219', '#8E6824', '#D8B36A', '#264B36', '#A8895A'];
  ...
  return (
    <div className="grid lg:grid-cols-5 gap-6">
      ...
      <div className="lg:col-span-3 bg-paper border border-ink/15 rounded-none p-6">
        <h2 className="text-xl font-bold text-ink mb-6">פרטי החישוב</h2>

        {/* Mode selector */}
        <div className="mb-5">
          <label className="block text-sm font-medium text-ink/70 mb-3">סוג חישוב</label>
          <div className="grid grid-cols-3 gap-2">
            {modes.map((m) => (
              <button
                ...
                className={`px-3 py-3 rounded-none border-2 transition text-right text-sm ${
                  mode === m.key
        ...
        {/* Amount */}
        <div className="mb-5">
          <label className="block text-sm font-medium text-ink/70 mb-2">{amountLabel}</label>
          <input
            type="number"
            min={0}
            step={100}
            ...
            className="w-full px-3 py-2 border border-ink/15 rounded-none focus:ring-2 focus:ring-gold text-lg text-right"
          />
        </div>

        {/* Rate */}
        <div className="mb-5">
          <label className="block text-sm font-medium text-ink/70 mb-2">שיעור מע&quot;מ (%)</label>
          <div className="flex gap-2 mb-2">
            {[0, 17, 18].map((r) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-none border text-sm font-medium transition ${
                  ratePercent === r
        ...
        {/* Formula explanation */}
        <div className="bg-cream-2 rounded-none p-4 text-sm">
          <p className="font-semibold text-ink mb-1">נוסחה</p>
          {mode === 'add' && (
      ...
      <div className="lg:col-span-2 space-y-4">
        ...
        <div className="bg-cream-2 border border-ink/15 rounded-none p-4">
          <h4 className="font-semibold text-ink mb-3">פירוט</h4>
          <dl className="space-y-2 text-sm">
            <div className="flex justify-between">
              <dt className="text-ink/70">סכום ללא מע&quot;מ:</dt>
              <dd className="font-medium">
                {formatCurrency(result?.amountWithoutVat ?? netResult?.net ?? 0)}
              </dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-ink/70">מע&quot;מ ({ratePercent}%):</dt>
            ...
            <div className="flex justify-between pt-2 border-t border-ink/15 font-bold">
              <dt className="text-ink">סה&quot;כ עם מע&quot;מ:</dt>
        ...
        {vatAmountDisplay > 0 && (
          <div className="bg-paper border border-ink/15 rounded-none p-4">
            <h4 className="font-semibold text-ink mb-2 text-sm">חלוקת הסכום</h4>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        >
          <p
            className={`text-xs font-medium mb-1 ${
              report.isRefund ? 'text-gold' : 'text-red-700'
            }`}
          >
          ...
          <p
            className={`text-2xl font-bold ${
              report.isRefund ? 'text-gold' : 'text-red-800'
            }`}
          >
        ...
        <div className="bg-cream-2 border border-ink/15 rounded-none p-4 text-center">
          <p className="text-xs text-ink/70 font-medium mb-1">מחזור תקופה</p>
          <p className="text-2xl font-bold text-ink">{formatCurrency(report.outputBase)}</p>
          <p className="text-xs text-ink/70 mt-0.5">כולל מע&quot;מ: {formatCurrency(report.outputGross)}</p>
        </div>
      </div>
      ...
      <div className="bg-paper border border-ink/15 rounded-none p-5">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-bold text-ink">עסקאות (מע&quot;מ עסקאות — Output)</h3>
          <button
            type="button"
            onClick={() => addRow('output')}
            className="text-sm bg-ink text-cream px-3 py-1.5 rounded-none hover:bg-ink-deep transition"
          >
            + הוסף עסקה
          </button>
        </div>
      ...
      <div className="bg-paper border border-ink/15 rounded-none p-5">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-bold text-ink">תשומות (מע&quot;מ תשומות — Input)</h3>
          <button
            type="button"
      ...
      {(report.outputVat > 0 || report.inputVat > 0) && (
        <div className="bg-paper border border-ink/15 rounded-none p-4">
          <h4 className="font-semibold text-ink mb-3">תרשים: עסקאות מול תשומות</h4>
          <ResponsiveContainer width="100%" height={200}>
            ...
            >
              ...
              <Bar dataKey="input" name='תשומות' fill="#102219" />
            </BarChart>
          </ResponsiveContainer>
        </div>
      )}
            ...
            <tr key={row.id}>
              <td className="py-1.5 pl-2">
                <input
                  ...
                  className="w-full px-2 py-1 border border-ink/15 rounded-none text-right text-xs"
                  placeholder="תיאור"
                />
              </td>
...
function TrackerTab() {
  ...
  return (
    <div className="space-y-6">
      <div className="bg-paper border border-ink/15 rounded-none p-5">
        <h3 className="font-bold text-ink mb-4">הוספה מהירה</h3>
        <div className="flex gap-3 flex-wrap">
          <div className="flex gap-2">
            <button
              type="button"
              onClick={() => setQuickType('output')}
              className={`px-3 py-2 rounded-none border text-sm font-medium ${
                quickType === 'output'
          ...
          <input
            ...
            className="flex-1 min-w-[120px] px-3 py-2 border border-ink/15 rounded-none text-right"
          />
          <button
            type="button"
            onClick={addQuick}
            className="px-4 py-2 bg-ink text-cream rounded-none hover:bg-ink-deep text-sm font-medium"
          >
            הוסף
          </button>
        </div>
        ...
        <>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <div className="bg-cream-2 border border-ink/15 rounded-none p-3 text-center">
              <p className="text-xs text-ink/70 mb-1">תשומות (כולל מע&quot;מ)</p>
              <p className="text-xl font-bold text-ink">
                {formatCurrency(summary.byType.input.grossAmount)}
              </p>
            </div>
            <div className={`border rounded-none p-3 text-center ${summary.netVatPosition >= 0 ? 'bg-red-50 border-red-200' : 'bg-cream-2 border-gold/40'}`}>
              <p className={`text-xs mb-1 ${summary.netVatPosition >= 0 ? 'text-red-700' : 'text-gold'}`}>
                {summary.netVatPosition >= 0 ? 'לתשלום' : 'להחזר'}
              </p>
              <p className={`text-xl font-bold ${summary.netVatPosition >= 0 ? 'text-red-800' : 'text-gold'}`}>
                {formatCurrency(Math.abs(summary.netVatPosition))}
              </p>
            </div>
            <div className="bg-cream-2 border border-ink/15 rounded-none p-3 text-center">
              <p className="text-xs text-ink/70 mb-1">חשבוניות</p>
              <p className="text-xl font-bold text-ink">{summary.totalInvoices}</p>
            </div>
          </div>
          ...
          <div className="bg-paper border border-ink/15 rounded-none overflow-hidden">
            <table className="w-full text-sm">
              <thead className="bg-cream-2">
                <tr className="text-right text-ink/70">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-2">
        {scenarioButtons.map((b) => (
          <button
            ...
            className={`px-3 py-3 rounded-none border-2 text-sm font-medium transition ${
              scenario === b.key
                ? 'border-ink bg-cream-2 text-ink'
                : 'border-ink/15 text-ink/70 hover:border-ink/30'
            }`}
      ...
      {scenario === 'discount' && (
        <div className="grid lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-3">
            ...
            <div className="bg-cream-2 border border-ink/15 rounded-none p-4">
              <h4 className="font-semibold text-ink mb-3">הנחה אחרי מע&quot;מ (על המחיר הסופי)</h4>
              <dl className="space-y-1 text-sm">
                <div className="flex justify-between">
                  <dt className="text-ink/70">מחיר מקורי (כולל מע&quot;מ):</dt>
                ...
                <div className="flex justify-between pt-1 border-t font-bold">
                  <dt>מחיר סופי לצרכן:</dt>
                  <dd className="text-gold">{formatCurrency(discountResult.discountAfterVat.finalGross)}</dd>
                </div>
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-none p-3 text-sm">
      ...
      {scenario === 'import' && (
        <div className="grid lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-3">
            ...
            />
            <div className="bg-paper border border-ink/15 rounded-none p-4">
              <h4 className="font-semibold text-ink mb-3">פירוט עלויות</h4>
              <dl className="space-y-2 text-sm">
      ...
      {scenario === 'industry' && (
        <div className="grid lg:grid-cols-2 gap-6">
          <div className="bg-paper border border-ink/15 rounded-none p-5">
            <h3 className="font-bold text-ink mb-4">כללי מע&quot;מ לפי ענף</h3>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-ink/70 mb-2">בחר ענף</label>
                <select
                  value={industry}
                  onChange={(e) => setIndustry(e.target.value as IndustryVatKey)}
                  className="w-full px-3 py-2 border border-ink/15 rounded-none text-right bg-paper"
                >
          ...
          <div className="space-y-3">
            ...
            >
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="font-bold text-ink">{industryRule.label}</span>
              </div>
              <p className="text-sm text-ink/70 mb-3">{industryRule.description}</p>
              <dl className="space-y-1 text-sm">
                <div className="flex justify-between">
                  <dt className="text-ink/70">שיעור מע&quot;מ:</dt>
                  <dd className="font-bold">{(industryRule.vatRate * 100).toFixed(0)}%</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-ink/70">מע&quot;מ על הסכום:</dt>
            ...
            <div className="bg-paper border border-ink/15 rounded-none p-4">
              <h4 className="font-semibold text-ink mb-2 text-sm">דוגמאות לענף זה</h4>
              <ul className="text-sm space-y-1">
                {industryRule.examples.map((ex, i) => (
                  <li key={i} className="flex gap-2 text-ink/70">
  ...
  return (
    <div className="grid lg:grid-cols-3 gap-6">
      ...
      <div className="space-y-5">
        <div className="bg-paper border border-ink/15 rounded-none p-5">
          <h3 className="font-bold text-ink mb-4">בחר סוג עוסק</h3>
          <div className="space-y-2">
            {operatorTypes.map((op) => (
              <button
                ...
                className={`w-full px-4 py-3 rounded-none border-2 text-right text-sm font-medium transition ${
                  selected === op.key
                    ? 'border-ink bg-cream-2 text-ink'
                    : 'border-ink/15 text-ink/70 hover:border-ink/30'
                }`}
      ...
      <div className="lg:col-span-2 space-y-4">
        <div className="bg-paper border border-ink/15 rounded-none p-5">
          <h3 className="font-bold text-ink text-lg mb-1">{rules.label}</h3>
          <p className="text-ink/70 text-sm mb-4">{rules.description}</p>

          <div className="grid grid-cols-2 gap-3 mb-4">
            <div className={`rounded-none p-3 text-center ${rules.collectsVat ? 'bg-red-50 border border-red-200' : 'bg-green-50 border border-green-200'}`}>
              <p className="text-xs font-medium mb-0.5 text-ink/70">גביית מע&quot;מ</p>
              <p className={`font-bold ${rules.collectsVat ? 'text-red-700' : 'text-green-700'}`}>
                {rules.collectsVat ? 'כן — גובה 18%' : 'לא'}
              </p>
            </div>
            <div className={`rounded-none p-3 text-center ${rules.canDeductInputVat ? 'bg-green-50 border border-green-200' : 'bg-cream-2 border border-ink/15'}`}>
              <p className="text-xs font-medium mb-0.5 text-ink/70">קיזוז תשומות</p>
              <p className={`font-bold ${rules.canDeductInputVat ? 'text-green-700' : 'text-ink/70'}`}>
          ...
          <div className="grid grid-cols-2 gap-3 mb-4">
            <div className="bg-cream-2 rounded-none p-3">
              <p className="text-xs text-ink/70 mb-0.5">דיווח</p>
              <p className="font-medium text-sm">{rules.reportingFrequency}</p>
            </div>
            {rules.collectsVat && (
              <div className="bg-cream-2 rounded-none p-3">
                <p className="text-xs text-ink/70 mb-0.5">מע&quot;מ שנתי על מחזור</p>
                <p className="font-bold text-ink text-sm">{formatCurrency(vatCost)}</p>
              </div>
            )}
          </div>

          <h4 className="font-semibold text-ink mb-2 text-sm">נקודות מפתח</h4>
...
function CompareTab() {
  ...
  return (
    <div className="space-y-6">
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-cream-2 border border-ink/15 rounded-none p-4">
              <p className="text-xs font-medium text-ink/70 mb-1">2024 — מע&quot;מ 17%</p>
            ...
            <div className="bg-cream-2 border border-gold/40 rounded-none p-4">
              <p className="text-xs font-medium text-gold mb-1">2025+ — מע&quot;מ 18%</p>
              <p className="text-2xl font-bold text-ink">
      ...
      {annualRevenue > 0 && (
        <div className="bg-paper border border-ink/15 rounded-none p-5">
          ...
          <div className="grid grid-cols-3 gap-4 mb-4">
            <div className="bg-cream-2 border border-ink/15 rounded-none p-3 text-center">
              <p className="text-xs text-ink/70 mb-1">מע&quot;מ שנתי 2024 (17%)</p>
              <p className="text-lg font-bold text-ink">
                {formatCurrency(annualRevenue * 0.17)}
              </p>
            </div>
            <div className="bg-cream-2 border border-gold/40 rounded-none p-3 text-center">
              <p className="text-xs text-gold mb-1">מע&quot;מ שנתי 2025+ (18%)</p>
              <p className="text-lg font-bold text-ink">
                {formatCurrency(annualRevenue * 0.18)}
              </p>
            </div>
            <div className="bg-red-50 border border-red-200 rounded-none p-3 text-center">
      ...
      {/* Chart */}
      <div className="bg-paper border border-ink/15 rounded-none p-4">
        <h4 className="font-semibold text-ink mb-3">השוואה גרפית</h4>
        <ResponsiveContainer width="100%" height={200}>
          <BarChart data={chartData}>
            ...
            <Tooltip formatter={(v) => formatCurrency(Number(v))} />
            <Legend />
            <Bar dataKey='ללא מע"מ' fill="#264B36" stackId="a" />
      ...
      {/* Transition note */}
      <div className="bg-cream-2 border border-ink/15 rounded-none p-4 text-sm text-ink/70">
...
export function VatCalculator() {
  ...
  return (
    <div>
      {/* Tab bar */}
      <div className="flex flex-wrap gap-1 mb-6 bg-cream-2 p-1 rounded-none">
        {tabs.map((t) => (
          <button
            key={t}
            type="button"
            onClick={() => setTab(t)}
            className={`flex-1 min-w-[calc(33%-4px)] px-3 py-2 rounded-none text-sm font-medium transition ${
              tab === t
                ? 'bg-ink text-cream'
