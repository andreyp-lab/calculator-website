...
function CustomTooltip({
  ...
  return (
    <div className="bg-paper border border-ink/15 rounded-none shadow-lg p-3 text-sm">
...
export function FireCalculator() {
  ...
  return (
    <div className="space-y-6" dir="rtl">
      {/* טאבים */}
      <div className="flex flex-wrap gap-1 border-b border-ink/15">
        {tabs.map((tab) => (
          <button
            key={tab.id}
        ...
        <div className="grid lg:grid-cols-5 gap-6">
          ...
          <div className="lg:col-span-3 space-y-5">
            <div className="bg-paper border border-ink/15 rounded-none p-6 space-y-5">
              <h2 className="text-xl font-bold text-ink">מחשבון FIRE - פרישה מוקדמת</h2>
                  ...
                  {(Object.entries(SWR_LABELS) as Array<[SWRType, string]>).map(
                    ([key, label]) => (
                      <button
                        ...
                        className={`px-3 py-2 rounded-none text-sm border-2 transition-colors text-right ${
                          input.swrType === key
              ...
              <div>
                <button
                  ...
                  className="text-sm text-gold hover:text-gold-2 font-medium flex items-center gap-1"
                >
                  {showAdvanced ? '▲' : '▼'} הגדרות מתקדמות - הקשר ישראלי
                </button>

                {showAdvanced && (
                  <div className="mt-4 space-y-4 bg-cream-2 rounded-none p-4 border border-ink/15">
                    <h3 className="text-sm font-bold text-ink">הקשר ישראלי</h3>
                    ...
                    <div className="flex items-center gap-3">
                      <input
                        ...
                        className="w-4 h-4"
                      />
                      <label htmlFor="bituachLeumi" className="text-sm text-ink/70">
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="bg-paper border border-ink/15 rounded-none p-4">
              <div className="flex justify-between items-center mb-2">
                <span className="text-sm font-medium text-ink/70">התקדמות ל-FIRE</span>
            ...
            <div className="bg-green-50 border-2 border-green-200 rounded-none p-5">
              <p className="text-sm font-medium text-ink/70 mb-1">סכום ה-FIRE שלך</p>
            ...
            <div className="bg-ink border-2 border-ink rounded-none p-5 text-cream">
              <p className="text-sm font-medium text-cream/80 mb-1">שנים עד פרישה</p>
            ...
            <div className="bg-paper border border-ink/15 rounded-none p-4 space-y-2 text-sm">
              <p className="font-semibold text-ink mb-2">פירוט הכנסות בפרישה</p>
              <div className="flex justify-between">
                <span className="text-ink/70">מתיק ההשקעות:</span>
              ...
              {result.passiveIncomeBreakdown.realEstate > 0 && (
                <div className="flex justify-between">
                  <span className="text-ink/70">מנדל&quot;ן:</span>
                  <span className="font-medium">
              ...
              {result.passiveIncomeBreakdown.bituachLeumi > 0 && (
                <div className="flex justify-between">
                  <span className="text-ink/70">ביטוח לאומי (מגיל 67):</span>
                  <span className="font-medium">
              ...
              <div className="flex justify-between border-t border-ink/15 pt-2">
                <span className="font-semibold text-ink">סה&quot;כ הכנסה פסיבית:</span>
            ...
            {!result.willReachFire && (
              <div className="bg-red-50 border border-red-300 rounded-none p-3 text-xs text-red-900">
          ...
          {/* גרף נתיב ל-FIRE */}
          <div className="lg:col-span-5 bg-paper border border-ink/15 rounded-none p-5">
            <h3 className="text-base font-semibold text-ink mb-4">נתיב ל-FIRE - שווי תיק לאורך השנים</h3>
            <ResponsiveContainer width="100%" height={300}>
              <AreaChart data={pathChartData} margin={{ top: 5, right: 20, left: 20, bottom: 5 }}>
                <defs>
                  <linearGradient id="portfolioGrad" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor="#8E6824" stopOpacity={0.3} />
                    <stop offset="95%" stopColor="#8E6824" stopOpacity={0.05} />
                  </linearGradient>
                </defs>
        ...
        <div className="space-y-6">
          <div className="grid md:grid-cols-1 gap-4">
            {result.fireTypeBreakdown.map((ft) => (
              <div
                key={ft.type}
                className="bg-paper border-2 rounded-none p-5"
                ...
                <div className="flex items-start justify-between flex-wrap gap-3">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      />
                      <h3 className="text-base font-bold text-ink">{ft.label}</h3>
                    </div>
          ...
          <div className="bg-paper border border-ink/15 rounded-none p-5">
            ...
            <ResponsiveContainer width="100%" height={280}>
              <BarChart data={fireTypesChartData} margin={{ top: 5, right: 20, left: 20, bottom: 5 }}>
                ...
                <YAxis tickFormatter={(v) => `${(v / 1_000_000).toFixed(1)}M`} tick={{ fontSize: 11 }} />
                <Tooltip content={<CustomTooltip />} />
                <Bar dataKey="FIRE Number" fill="#8E6824" radius={[4, 4, 0, 0]}>
                  {fireTypesChartData.map((_, index) => {
                    const colors = ['#10b981', '#8E6824', '#264B36', '#f59e0b', '#D8B36A'];
        ...
        <div className="space-y-6">
          <div className="grid md:grid-cols-2 gap-4">
            {result.swrScenarios.map((scenario) => (
              <div
                key={scenario.type}
                className={`bg-paper border-2 rounded-none p-5 transition-colors ${
              ...
              >
                <div className="flex justify-between items-start mb-3">
                  <h3 className="text-sm font-bold text-ink">{scenario.label}</h3>
                  <span className="text-lg font-bold text-gold">{scenario.rate}%</span>
                </div>
          ...
          <div className="bg-paper border border-ink/15 rounded-none p-5 space-y-4">
            <h3 className="text-base font-bold text-ink">מחקר Trinity - הבסיס של כלל ה-4%</h3>
            <div className="grid md:grid-cols-2 gap-4 text-sm text-ink/70">
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-none p-5 space-y-4">
            ...
            <div className="grid md:grid-cols-2 gap-4 text-sm">
              ...
              <div className="space-y-3">
                <div className="flex items-start gap-2">
                  <span className="text-ink-mid font-bold mt-0.5">ℹ</span>
                  <div>
      ...
      {activeTab === 'withdrawal' && (
        <div className="space-y-6">
              ...
              >
                <defs>
                  <linearGradient id="withdrawalGrad" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor="#264B36" stopOpacity={0.3} />
                    <stop offset="95%" stopColor="#264B36" stopOpacity={0.05} />
                  </linearGradient>
                </defs>
                <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                <XAxis dataKey="name" tick={{ fontSize: 11 }} interval={4} />
          ...
          {/* האם התיק שורד? */}
          <div className="bg-paper border border-ink/15 rounded-none p-5">
            <h3 className="text-sm font-bold text-ink mb-3">האם התיק שורד?</h3>
          ...
          {/* טיפים */}
          <div className="bg-cream-2 border border-ink/15 rounded-none p-5 text-sm text-ink/70 space-y-2">
      ...
      {activeTab === 'sensitivity' && (
        <div className="space-y-6">
              ...
              {[
                {
                  ...
                  age: result.fireAge,
                  color: 'bg-cream-2 border-ink/15',
                  badge: 'bg-gold',
                },
                {
                ...
                <div
                  key={row.label}
                  className={`flex items-center justify-between rounded-none border p-4 ${row.color}`}
                >
                  <div className="flex items-center gap-3">
          ...
          <div className="bg-paper border border-ink/15 rounded-none p-5">
            ...
            <ResponsiveContainer width="100%" height={250}>
              ...
              >
                ...
                <Tooltip />
                <Bar dataKey="שנים" fill="#8E6824" radius={[4, 4, 0, 0]}>
                  {[
                    '#8E6824',
                    '#10b981',
                    '#264B36',
          ...
          {/* המלצות */}
          <div className="bg-cream-2 border border-ink/15 rounded-none p-5 text-sm space-y-3">
            <h3 className="font-bold text-ink">המלצות לזרז FIRE</h3>
            <div className="grid md:grid-cols-2 gap-3">
              <div className="bg-paper rounded-none p-3 border border-ink/15">
                <p className="font-semibold text-ink mb-1">⬆ הגדל הכנסה</p>
              ...
              <div className="bg-paper rounded-none p-3 border border-ink/15">
                <p className="font-semibold text-ink mb-1">⬇ הקטן הוצאות קבועות</p>
                <p className="text-ink/70 text-xs">
              ...
              <div className="bg-paper rounded-none p-3 border border-ink/15">
                <p className="font-semibold text-ink mb-1">📈 אופטימיזציה מסית</p>
                <p className="text-ink/70 text-xs">
              ...
              <div className="bg-paper rounded-none p-3 border border-ink/15">
                <p className="font-semibold text-ink mb-1">🏠 נדל"ן כרכיב FIRE</p>
                <p className="text-ink/70 text-xs">
