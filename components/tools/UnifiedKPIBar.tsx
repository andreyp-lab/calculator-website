...
export function UnifiedKPIBar() {
  ...
  return (
    <div className="bg-ink text-cream overflow-hidden">
      <div className="px-3 py-2 border-b border-cream/10 flex items-center justify-between">
        <span className="text-xs font-semibold text-cream/80">📊 KPI חי - מתעדכן בזמן אמת</span>
...
function KPI({
  ...
  value: string;
  subValue?: string;
  color: 'emerald' | 'red' | 'amber' | 'gold' | 'gray';
}) {
  ...
  return (
    <div className="px-3 py-2 text-center">
      <div className="flex items-center justify-center gap-1 text-[10px] text-cream/60 mb-0.5">
        <Icon className="w-3 h-3" />
        <span>{label}</span>
      </div>
      <div className={`text-lg font-bold ${valueColor}`}>{value}</div>
      {subValue && <div className="text-[10px] text-cream/45">{subValue}</div>}
    </div>
  );
}
