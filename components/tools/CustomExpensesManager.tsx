...
const CATEGORIES: Array<{
  ...
  { value: 'supplier', label: 'ספק', color: 'bg-cream-2 text-ink', icon: '🚚' },
  ...
  { value: 'check', label: 'שיק לתשלום', color: 'bg-cream-2 text-ink', icon: '✍️' },
  ...
  { value: 'other', label: 'אחר', color: 'bg-cream-2 text-ink', icon: '📋' },
];
    ...
    <div className="bg-paper border-2 border-ink/15 p-5 shadow-sm">
      <div className="flex items-center justify-between mb-3">
        ...
        <button
          ...
          className="flex items-center gap-1 px-2.5 py-1 bg-ink text-cream hover:bg-ink-deep text-xs"
        >
      ...
      {cashFlow.customExpenses.length > 0 && (
        <div className="bg-cream-2 border border-ink/15 p-2 mb-3 text-sm">
          <div className="flex justify-between">
            <span className="text-ink/70">סה"כ הוצאות:</span>
            <span className="font-bold text-ink">{fmt(total)}</span>
          </div>
        </div>
      )}

      {showForm && (
        <div className="bg-cream-2 border border-ink/15 p-3 mb-3 space-y-3">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            <div>
              ...
              <select
                ...
                className="w-full px-2 py-1.5 border border-ink/15 text-sm"
              >
                {CATEGORIES.map((c) => (
          ...
          {form.category === 'supplier' && (
            <div className="bg-paper border border-ink/15 p-2">
              <div className="text-xs font-semibold text-ink mb-2">פרטי ספק</div>
              <div>
                <label className="block text-xs text-ink/70 mb-1">סטטוס</label>
                <select
                  value={form.status}
                  onChange={(e) =>
          ...
          {form.category === 'authority' && (
            <div className="bg-red-50 border border-red-200 p-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-xs text-ink/70 mb-1">סוג תשלום</label>
                  <select
                ...
                <div>
                  <label className="block text-xs text-ink/70 mb-1">דחיפות</label>
                  <select
          ...
          {form.category === 'creditcard' && (
            <div className="bg-amber-50 border border-amber-200 p-2">
              ...
              <div className="grid grid-cols-3 gap-2">
                <div>
                  <label className="block text-xs text-ink/70 mb-1">חברת אשראי</label>
                  <select
          ...
          <div className="flex gap-2">
            <button
              ...
              className="px-3 py-1 bg-ink text-cream hover:bg-ink-deep text-xs"
            >
              {editingId ? 'עדכן' : 'הוסף'}
            </button>
            <button onClick={reset} className="px-3 py-1 bg-cream-2 text-ink text-xs">
              ביטול
            </button>
          </div>
        </div>
        ...
        <div className="space-y-3">
          {byCategory.map((cat) => (
            <div key={cat.value} className="space-y-1.5">
              <div className="flex items-center justify-between text-xs font-semibold text-ink/70 border-b border-ink/15 pb-1">
                <span>
                ...
                return (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="p-1 text-gold hover:bg-cream-2"
                    >
