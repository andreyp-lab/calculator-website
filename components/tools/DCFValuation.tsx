...
export function DCFValuation() {
  ...
  return (
    <div className="space-y-4">
      ...
      {error && (
        <div className="bg-red-50 border-2 border-red-200 p-3 text-red-900 text-sm flex items-center gap-2">
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        <div className="bg-paper border-2 border-ink/15 shadow-sm overflow-hidden">
          ...
          <div className="p-3 space-y-2 text-sm">
            ...
            <div className="bg-cream-2 p-2 text-xs">
            ...
            <div className="bg-cream-2 border-2 border-ink/15 p-2 text-center">
              <div className="text-xs text-ink/70">WACC</div>
              <div className="text-2xl font-bold text-ink">{wacc.toFixed(2)}%</div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-paper border-2 border-ink/15 shadow-sm overflow-hidden">
          <div className="bg-emerald-600 text-white p-2 text-sm font-bold flex items-center justify-between">
            ...
            {budget && settings && (
              <button
                ...
                className="text-xs bg-white/20 hover:bg-white/30 px-2 py-0.5 flex items-center gap-1"
              >
          ...
          <div className="p-3 space-y-2 text-sm">
            ...
            <div className="flex gap-1">
              <button
                onClick={addFCFYear}
                className="px-2 py-1 bg-ink text-cream text-xs hover:bg-ink-deep"
              >
                + שנה
              </button>
              <button
                onClick={removeFCFYear}
                className="px-2 py-1 bg-cream-2 text-ink text-xs"
              >
                − שנה
              </button>
            </div>
      ...
      <div className="bg-paper border-2 border-ink/15 shadow-sm overflow-hidden">
        ...
        <div className="p-3 grid md:grid-cols-2 gap-3">
          <div>
            <label className="block text-xs text-ink/70 mb-1">שיטה</label>
            <select
          ...
          <div className="p-4 space-y-2 text-sm">
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <div className="mt-2 bg-cream-2 p-2 space-y-1 text-xs">
        ...
        <div className="bg-paper border-2 border-ink/15 shadow-sm overflow-hidden">
          ...
          <div className="p-3 overflow-x-auto">
            <table className="w-full text-xs border-collapse">
              <thead>
                <tr>
                  <th className="border border-ink/15 p-1 bg-cream-2">WACC \ g</th>
                  {sensitivity[0].map((cell, i) => (
                    <th key={i} className="border border-ink/15 p-1 bg-cream-2">
              ...
              <tbody>
                {sensitivity.map((row, i) => (
                  <tr key={i}>
                    <td className="border border-ink/15 p-1 bg-cream-2 font-bold text-center">
                    ...
                    {row.map((cell, j) => {
                      ...
                      return (
                        <td
                          key={j}
                          className={`border border-ink/15 p-1 text-center ${
                            isBaseRow
...
function Field({
  ...
  return (
    <div>
      <label className="block text-xs text-ink/70 mb-0.5">{label}</label>
      <input
        type="number"
        value={value}
        onChange={(e) => onChange(parseFloat(e.target.value) || 0)}
        step={step ?? 1}
        className="w-full px-2 py-1 border border-ink/15 text-sm"
      />
    </div>
  );
}
